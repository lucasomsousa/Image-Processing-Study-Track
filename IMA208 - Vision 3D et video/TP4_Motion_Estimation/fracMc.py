...
def fracMc(ref,mvf,outofbound=20):
    ...
    [paddedRows, paddedCols] = np.shape(ref)
    colMeshGrid, rowMeshGrid = np.meshgrid(np.arange(paddedCols), np.arange(paddedRows)) # Original coordinates, in the padded image

    # Put the input coordinates into the form of an array, to match scipy format
    ...
    # Put the coordinates in which we want ot do the interpolation into the form of an array, to match scipy format
